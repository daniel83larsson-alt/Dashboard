...
import { isDoneInCurrentPeriod, currentHabitStreak, intervalLabel, habitColor, type Habit, type HabitLog } from '@/lib/habits'
      ...
      {habits.length > 0 && (
        <div className="flex flex-col gap-2">
          {habits.map((h, i) => {
            ...
            // Varje vana får en egen, stabil färg — samma färg syns igen som
            // prick i kalendern, så "Kreatin" alltid är samma färg överallt
            // istället för att alla vanor ser likadana ut (Daniel: tydligare
            // rader).
              ...
              <div key={h.id} className="bg-bg rounded-xl p-3 flex items-center gap-3">
                <button
                  ...
                  className={`w-6 h-6 flex-shrink-0 rounded-lg border flex items-center justify-center text-xs font-bold transition-colors disabled:opacity-50 ${
                    done ? `${color.bg} ${color.border} text-bg` : `${color.borderMuted} text-transparent ${color.hoverBorder}`
                  }`}
                >
                  ✓
                </button>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] text-muted mt-0.5">
                    ...
                    {streak > 0 && <span className={`${color.text}`}> · {streak} i rad</span>}
                  </div>
                </div>
          ...
          <p className="text-muted text-[10px] mt-0.5">Missat en dag? Kryssa i den i kalendern längre ner.</p>
        </div>
      )}
