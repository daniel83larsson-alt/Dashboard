...
- 🔄 **Homey-koppling — kod klar, väntar på två steg från Daniel.** Bara läsa (inte styra), byggd mot Athoms officiella `homey-api`-SDK (verifierat mot källkoden, inte gissat). Client ID/Secret mottagna och pushade i koden (aldrig i git). Kvar innan det går att testa: 1) köra uppdaterade `schema.sql` i Supabase (ny tabell), 2) lägga in HOMEY_CLIENT_ID/SECRET som miljövariabler i Vercel-dashboarden (jag kan inte göra det själv just nu, se öppen fråga om Vercel-inloggning ovan).
