...
- 🔄 **Homey-koppling — driftsatt och verifierad, väntar bara på att Daniel klickar Anslut.** Bara läsa (inte styra), byggd mot Athoms officiella `homey-api`-SDK (verifierat mot källkoden, inte gissat). Databastabell körd och kontrollerad direkt mot Supabase. Client ID/Secret inlagda som skyddade miljövariabler i Vercel (Daniel skapade en tidsbegränsad åtkomsttoken åt mig för det). Ny produktionsdeploy verifierad live: `/api/homey/authorize` skickar korrekt vidare till Homeys inloggning med rätt client_id. Sista steget kräver Daniels egen Homey-inloggning och kan inte göras av mig — testa "Anslut Homey" på Översikt.
