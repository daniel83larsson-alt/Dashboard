...
- **Kod som är rätt är inte samma sak som att den faktiskt körs.** Allt som beror på något UTANFÖR själva koden — en databasmigrering, ett cron-schema, en miljövariabel, vilken gren som faktiskt är produktion/schemaläggs — ska verifieras direkt mot den levande resursen innan något markeras ✅, inte bara mot att koden klarar tsc/lint/tester/bygge. (Retro, 2026-09-20: två separata incidenter samma session — en migrering som aldrig applicerades i produktion, och ett cron-schema som bara lades till på fel gren — båda klarade hela den vanliga testsviten men gjorde ingenting på riktigt i över en vecka.)
